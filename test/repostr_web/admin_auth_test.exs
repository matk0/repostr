defmodule RepostrWeb.AdminAuthTest do
  use RepostrWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Repostr.Accounts
  alias RepostrWeb.AdminAuth
  import Repostr.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RepostrWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_admin/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RepostrWeb.Endpoint.subscribe(live_socket_id)
